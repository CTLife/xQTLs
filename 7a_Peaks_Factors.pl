...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "4a_keptPeaks"   
my $output_g = '';  ## such as "7a_Peaks_Factors"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------     
        Usage:
               perl 7a_Peaks_Factors.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]     [-known knownFactors] 
        For instance:
               nohup time  perl 7a_Peaks_Factors.pl   -in 4a_keptPeaks   -out 7a_Peaks_Factors   -known  PCA_PEER/BA9.knownFactors.all.txt > 7a_Peaks_Factors.runLog.txt  2>&1    &
...
$input_g  = '4a_keptPeaks';          ## This is only an initialization value or suggesting value, not default value.
$output_g = '7a_Peaks_Factors';      ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl 7a_Peaks_Factors.pl  -help' \n";
    exit 0;
}
...
open(INPUT_3, "<", "5a_PCA_Peaks/merged.pca" ) or die;  
...
for(my $n=1; $n<=$count; $n++) {
    my $m = $n + 1;
    system( "head -n $m  5a_PCA_Peaks/merged.pca               > $output_g/PCA/PCA.$n.txt"  );
    system( "cat   $output_g/PCA/PCA.$n.txt  $known_g.nohead   > $output_g/PCA/PCA.withKnown.$n.txt"  );
}

&myMakeDir("$output_g/Correlation");
system( "Rscript PCA_PEER/3_heatmap.R  --knownFactors=$known_g  --peerFactors=$output_g/PEER/factors.T.backup.txt    --PCAs=5a_PCA_Peaks/merged.pca   --genotypes=6_PCA_SNP/SNPs.vcf.pca    --output=$output_g/Correlation "  );                                                    
