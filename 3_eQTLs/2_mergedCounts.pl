...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "1_TPM_Counts/1_BA9/Input/featureCounts.forward "   
my $output_g = '';  ## such as "2_mergedCounts/1_BA9/Input/featureCounts.forward "
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------     
        Usage:
               perl 2_mergedCounts.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]      
        For instance:
               nohup time  perl 2_mergedCounts.pl   -in 1_TPM_Counts/1_BA9/Input/featureCounts.forward   -out 2_mergedCounts/1_BA9/Input/featureCounts.forward      > 2_mergedCounts.1.runLog.txt  2>&1    &
...
$input_g  = '1_TPM_Counts/1_BA9/Input/featureCounts.forward';          ## This is only an initialization value or suggesting value, not default value.
$output_g = '2_mergedCounts/1_BA9/Input/featureCounts.forward';         ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl 2_mergedCounts.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp2 =~ s/\.IP$//  ;
        $temp2 =~ s/\.IN$//  ;
        print FH2 "#chr\tstart\tend\tname\tlength\tstrand\t$temp2\n";
