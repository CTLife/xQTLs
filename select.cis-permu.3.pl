...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------     
        Usage:
               perl select.cis-permu.3.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]      
        For instance:
               nohup time  perl select.cis-permu.3.pl   -in 8a_cis_permutation   -out 8a_cis_permutation.Selected     > selected.runLog.txt  2>&1    &
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl select.cis-permu.3.pl  -help' \n";
    exit 0;
}
...
while( my $filename1 = readdir($DH1) ) {
    if($filename1 =~ m/^\d/) {
                ...
                for ( my $j=0; $j<=$#lines1; $j++ ) {
        	        ...
        	        if( $adj_beta_pval < 0.05 ) { print $DH4a  $lines1[$j]; }    
        	        if( $adj_beta_pval < 0.05 ) { print $DH4b  "$var_chr\t$var_from\t$var_to\t$var_id\t$dist\n"; }    
        	        if( $adj_beta_pval < 0.05 ) { print $DH4c  "$phe_chr\t$phe_from\t$phe_to\t$phe_id\t$dist\t$phe_strd\n"; }    
                }
                close($DH4a);
                close($DH4b);
                close($DH4c);
